/* $Id: xuartns550_sinit.c,v 1.1 2006/02/17 22:43:40 moleres Exp $ */
/*****************************************************************************
*
...
*           XUartNs550 instance. Passing in a device id associates the generic
*           XUartNs550 instance to a specific device, as chosen by the caller
...
XStatus XUartNs550_Initialize(XUartNs550 *InstancePtr, Xuint16 DeviceId)
{
    ...
    if (ConfigPtr == (XUartNs550_Config *)XNULL)
    {
        return XST_DEVICE_NOT_FOUND;
    }

    ConfigPtr->DefaultBaudRate = XPAR_DEFAULT_BAUD_RATE;
    return XUartNs550_CfgInitialize(InstancePtr, ConfigPtr, ConfigPtr->BaseAddress);
}
