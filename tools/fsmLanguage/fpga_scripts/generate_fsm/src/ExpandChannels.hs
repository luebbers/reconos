...
a set of guarded assignment statements
--}

module ExpandChannels(elaborateChannelsBody, fullSig, existsSig, channelReadEnableSig, channelWriteEnableSig, channelInSig, channelOutSig) where
...
writeGuardStay (Channel_access wc) = (BinaryOp "/=" (Var_access $ JustVar (fullSig wc)) (Const_access $ V_tick '0'))
...
writeGuardComplete (Channel_access wc) = (BinaryOp "=" (Var_access $ JustVar (fullSig wc)) (Const_access $ V_tick '0'))
...
readGuardStay (Channel_access wc) = (BinaryOp "=" (Var_access $ JustVar (existsSig wc)) (Const_access $ V_tick '0'))
...
readGuardComplete (Channel_access wc) = (BinaryOp "/=" (Var_access $ JustVar (existsSig wc)) (Const_access $ V_tick '0'))
...
genReadStatements (Assign_stmt l r@(Channel_access n)) =
	[
	Assign_stmt l (Var_access (JustVar (channelOutSig n))),
...
getRHS (Func_stmt e) = (Var_access (JustVar "NULL_NO_LHS"))
getLHS (Assign_stmt a b) = a
getLHS (VAssign_stmt a b) = a
