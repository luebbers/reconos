...
//            *y++ = *x++;
//        }
        if (write(memfd, (char*)x, 8192) != 8192) {
            perror("error while writing data to buf8k");
        }
...
//            *y++ = *x++;
//        }
        if (read(memfd, (char*)y, 8192) != 8192) {
            perror("error while reading data from buf8k");
        }
...
int main( int argc, char *argv[] )
{
    ...
    t_start = gettime();
    t_stop = gettime();
    t_overhead = calc_timediff_ms( t_start, t_stop );

/*    memfd = open("/dev/buf8k", O_RDWR);
    if (memfd < 0) {
        perror("error while opening buf8k device");
    }*/
    ...
    t_stop = gettime(  );
    t_gen = calc_timediff_ms( t_start, t_stop ) - t_overhead;
    printf( "done\n" );
    ...
    for ( i = 0; i < MT_HW_NUM_SW_THREADS; i++ ) {
        ...
        pthread_setschedparam(thread_sorter[i], SCHED_FIFO, &sp);
    }
...
*/
    ...
    t_stop = gettime(  );
    t_sort += calc_timediff_ms( t_start, t_stop ) - t_overhead;
    ...
    t_stop = gettime(  );
    t_merge = calc_timediff_ms( t_start, t_stop ) - t_overhead;
    printf( "done\n" );
    ...
    t_stop = gettime(  );
    t_check = calc_timediff_ms( t_start, t_stop ) - t_overhead;
