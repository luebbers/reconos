...
<xsl:template name="_calc_Proc_Height">
	...
	<xsl:variable name="tot_bifs_h_">
		...
		<xsl:if test="/EDKSYSTEM/BLKDSHAPES/PROCSHAPES/MODULE[(@INSTANCE = $iProcInst)]/@BIFS_H">
			...
			<xsl:value-of select="(($BLKD_BIF_H + $BLKD_BIF_GAP_H) * $bifs_h_)"/>	
		</xsl:if>
	</xsl:variable>	
	
	<xsl:value-of select="(($BLKD_MOD_LANE_H * 2) + $tot_bifs_h_ + ($BLKD_MOD_LABEL_H + $BLKD_BIF_GAP_H))"/>	
</xsl:template>
...
<!--			
			<xsl:message>Found stack of height <xsl:value-of select="$stack_height_"/></xsl:message>
			<xsl:message>==============================</xsl:message>
-->			
		...
		<xsl:for-each select="/EDKSYSTEM/BLKDSHAPES/CMPLXSHAPES/CMPLXSHAPE/BUSCONNS/BUSCONNLANE[(@BUSNAME = $iBusName)]/BUSCONN">
			...
			<xsl:variable name="peri_cstk_y_">
				...
				<xsl:variable name="peri_bif_dy_">
					<xsl:value-of select="(($BLKD_BIF_H + $BLKD_BIF_GAP_H)  * @BIF_Y)"/>
				</xsl:variable>
				
				<xsl:variable name="peri_bc_y_">
					<xsl:value-of select="($BLKD_MOD_LANE_H + $BLKD_MOD_LABEL_H + $BLKD_BIF_GAP_H + $peri_bif_dy_ + ceiling($BLKD_BIF_H div 2)) - ceiling($BLKD_BIFC_H div 2)"/>
				</xsl:variable>
	...
	<xsl:variable name="spaceWidth_">
		<xsl:choose>
			<xsl:when test="/EDKSYSTEM/BLKDSHAPES/BCLANESPACES/BCLANESPACE[((@EAST = $iStackToEast) or (not($iStackToWest = 'NONE') and (@WEST = $iStackToWest)))]">
				<xsl:value-of select="((/EDKSYSTEM/BLKDSHAPES/BCLANESPACES/BCLANESPACE[((@EAST = $iStackToEast) or (not($iStackToWest = 'NONE') and (@WEST = $iStackToWest)))]/@BUSLANES_W + 1) * $BLKD_BUS_LANE_W)"/>
			</xsl:when>	
			<xsl:otherwise>0</xsl:otherwise>	
		</xsl:choose>	
	</xsl:variable>	
