...
<xsl:template name="Define_FreeCmplxModules">
	...
	<xsl:for-each select="BLKDSHAPES/CMPLXSHAPES/CMPLXSHAPE[(@IS_PROMOTED and not(@IS_PENALIZED) and not(@STACK_INDEX))]">	
		120GG
		<xsl:variable name="cmplxId_" sele1200Gct="position()"/>
...
-->	
	...
	<xsl:variable name="label_y_">
		<xsl:value-of select="$BLKD_MOD_LANE_H + $BLKD_BIF_H + $BLKD_BIFC_H +  $BLKD_BIF_GAP_V"/>	
	</xsl:variable>
...
-->	
	...
	<xsl:variable name="label_y_">
		<xsl:choose>
			<xsl:when test="$iModVori = 'rot180'">
				<xsl:value-of select="($BLKD_MOD_LANE_H + (($BLKD_BIF_H + $BLKD_BIF_GAP_V) * $bifs_h_))"/>	
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="$BLKD_MOD_LANE_H"/>	
			</xsl:otherwise>
	...
	<xsl:variable name="bif_dy_">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of select="($BLKD_MOD_LANE_H + $BLKD_MOD_LABEL_H + $BLKD_BIF_GAP_V)"/>	
			</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
...
-->
		...
		<xsl:for-each select="/EDKSYSTEM/MODULES/MODULE[@INSTANCE=$iModInst]/BUSINTERFACE[(@BIF_X and @BIF_Y and not(@BUSNAME = '__NOC__'))]">
			...
			<xsl:variable name="bif_y_">
				<xsl:value-of select="(($BLKD_BIF_H + $BLKD_BIF_GAP_V)  * @BIF_Y)"/>
			</xsl:variable>
			...
			<xsl:variable name="bif_x_" >
				<xsl:if test="not(@ORIENTED='CENTER')">
					<xsl:value-of select="(($BLKD_BIF_W * @BIF_X) + ($BLKD_BIF_GAP_H * @BIF_X) + ($BLKD_MOD_LANE_W * 1))"/>
				</xsl:if>
...
-->	
	...
	<xsl:if test="(/EDKSYSTEM/BLKDSHAPES/CMPLXSHAPES/CMPLXSHAPE/MODULE[(@INSTANCE = $iShapeInst)]/@BIFS_H)">
		...
		<xsl:value-of select="($BLKD_MOD_LABEL_H + ($BLKD_BIF_H * $bifs_h_) + ($BLKD_BIF_GAP_V * $bifs_h_) + ($BLKD_MOD_LANE_H * 2))"/>
	</xsl:if>
...
-->	
	...
	<xsl:if test="(/EDKSYSTEM/BLKDSHAPES/CMPLXSHAPES/CMPLXSHAPE[(@SHAPE_ID = $iShapeId)]/MODULE/@BIFS_H)">
		...
		<xsl:value-of select="($BLKD_MOD_LABEL_H + ($BIF_H * $bifs_h_) + ($BLKD_BIF_GAP_V * $bifs_h_) + ($BLKD_MOD_LANE_H * 2))"/>
	</xsl:if>
