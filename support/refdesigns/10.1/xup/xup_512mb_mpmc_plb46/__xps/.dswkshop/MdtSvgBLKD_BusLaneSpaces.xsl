...
-->						
	...
	<xsl:variable name="procBif_Y_"    select="((($BLKD_BIF_H + $BLKD_BIF_GAP_V) * BUSCONN/@BIF_Y) + ($BLKD_MOD_LANE_H + $BLKD_MOD_LABEL_H + $BLKD_BIF_GAP_V))"/>
	...
	<xsl:variable name="v_bus_ul_x_">
		<xsl:choose>
			<xsl:when test="@ORIENTED='WEST'">
				<xsl:value-of select="($vert_line_x_ + $BLKD_BIF_GAP_H)"/>
			</xsl:when>
...
-->	
	...
	<xsl:variable name="cmplxBif_Y_"    select="((($BLKD_BIF_H + $BLKD_BIF_GAP_V) * BUSCONN/@BIF_Y) + ($BLKD_MOD_LANE_H + $BLKD_MOD_LABEL_H + $BLKD_BIF_GAP_V))"/>
		...
		<xsl:for-each select="BUSCONN">
			...
			<xsl:choose>
				<xsl:when test="@IS_PROCCONN and @BIF_Y">
					...
					<xsl:variable name="procBif_Y_"    select="((($BLKD_BIF_H + $BLKD_BIF_GAP_V) * @BIF_Y) + ($BLKD_MOD_LANE_H + $BLKD_MOD_LABEL_H + $BLKD_BIF_GAP_V))"/>
					...
					<xsl:variable name="memcConn_Y_">
						<xsl:choose>
							<xsl:when test="(/EDKSYSTEM/BLKDSHAPES/CMPLXSHAPES/CMPLXSHAPE[MODULE[(@INSTANCE = $memcInstance_)]]/@MODS_H = 1)">
								<xsl:value-of  select="($memshp_Y_ + ($BLKD_MOD_LANE_H + $BLKD_MOD_LABEL_H + $BLKD_BIF_GAP_V) +  ($memcBif_Y_ * ($BLKD_BIF_H + $BLKD_BIF_GAP_V)) + ceiling($BIF_H div 2) - ceiling($BLKD_BIFC_H div 2) + $cmplxStack_H_diff_)"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of  select="($memshp_Y_ + $BLKD_MOD_H + $BLKD_MOD_LANE_H + ($memcBif_Y_ * ($BLKD_BIF_H + $BLKD_BIF_GAP_V)) + ceiling($BLKD_BIF_H div 2) - ceiling($BLKD_BIFC_H div 2) + $cmplxStack_H_diff_)"/>
							</xsl:otherwise>
						</xsl:choose>
					</xsl:variable>
...
<!--
		<xsl:choose>
			<xsl:when test="((@X &lt; ($bus_x_ + $BLKD_BUS_ARROW_W)) and (@BIFSIDE ='0'))">
				<xsl:message>Adjusting...</xsl:message>
			</xsl:when>
		</xsl:choose>
-->		
...
<!--		
 		...
 		<xsl:message>bus_X_      <xsl:value-of select="$bus_x_"/></xsl:message>
 		<xsl:message>bif side    <xsl:value-of select="@BIFSIDE"/></xsl:message>
 		<xsl:message>h_bus_ul_x  <xsl:value-of select="$h_bus_ul_x_"/></xsl:message>
 		<xsl:message>h_bus_ul_x_dx_  <xsl:value-of select="$h_bus_ul_x_dx_"/></xsl:message>
 		<xsl:message>h_bus_width <xsl:value-of select="$h_bus_width_"/></xsl:message>
 		<xsl:message>h_bus_ul_y  <xsl:value-of select="$h_bus_ul_y_"/></xsl:message>
-->		
...
<!-- 	
	...
	<xsl:for-each select="exsl:node-set($memConn_heights_)/MEMCONN">
 		<xsl:message>mc_X_       <xsl:value-of select="@X"/></xsl:message>
	</xsl:for-each>
	<xsl:message>bcInSpace_X_       <xsl:value-of select="($bcInSpace_X_ + $BLKD_P2P_BUS_W)"/></xsl:message>
...
-->						
					...
					<xsl:otherwise>
						...
						<xsl:variable name="modBc_Y_"         select="((($BLKD_BIF_H + $BLKD_BIF_GAP_V) * $modBif_Y_) + ($BLKD_MOD_LANE_H + $BLKD_MOD_LABEL_H + $BLKD_BIF_GAP_V))"/>
					...
					<xsl:variable name="modBc_Y_">
						<xsl:choose>
							<xsl:when test="($modType_ = 'MEMORY_CNTLR') and (($modBusStd_ = 'LMB') or ($modBusStd_= 'OCM'))">
					      		<xsl:value-of select="$BLKD_MOD_H + $BLKD_MOD_LANE_H + ((($BLKD_BIF_H + $BLKD_BIF_GAP_V) * $modBif_Y_))"/>
							</xsl:when>
							<xsl:otherwise>
					      		<xsl:value-of select="((($BLKD_BIF_H + $BLKD_BIF_GAP_V) * $modBif_Y_) + ($BLKD_MOD_LANE_H + $BLKD_MOD_LABEL_H + $BLKD_BIF_GAP_V))"/>
							</xsl:otherwise>
						</xsl:choose>
					</xsl:variable>   
<!--					
					...
					<xsl:message><xsl:value-of select="$multiInstance_"/>.<xsl:value-of select="$bifName_"/>:TcY = <xsl:value-of select="($BLKD_MOD_LANE_H + $BLKD_MOD_LABEL_H + $BLKD_BIF_GAP_V)"/></xsl:message>
-->	
...
-->				
		...
		<xsl:variable name="splitBif_Y_"    select="((($BLKD_BIF_H + $BLKD_BIF_GAP_V) * BUSCONN/@BIF_Y) + ($BLKD_MOD_LANE_H + $BLKD_MOD_LABEL_H + $BLKD_BIF_GAP_V))"/>
...
-->	
					<xsl:value-of select="($splitshp_Y_ + $BLKD_MOD_H + $BLKD_BIF_GAP_V + (BUSCONN/@BIF_Y * ($BLKD_BIF_H + $BLKD_BIF_GAP_V)) + ceiling($BLKD_BIF_H div 2) - ceiling($BLKD_BIFC_H div 2) + $splitStack_H_diff_)"/>
				</xsl:when>     
				<xsl:otherwise>
