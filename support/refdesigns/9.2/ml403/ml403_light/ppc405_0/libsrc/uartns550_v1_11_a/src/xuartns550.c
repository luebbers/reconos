/* $Id: xuartns550.c,v 1.3 2007/09/13 23:08:02 moleres Exp $ */
/*****************************************************************************
*
					...
					u32 EffectiveAddr)
{
	 ...
	 * level
	 */
	XIo_Out8(InstancePtr->BaseAddress + XUN_FCR_OFFSET, XUN_FIFO_ENABLE);
...
unsigned int XUartNs550_SendBuffer(XUartNs550 *InstancePtr)
{
	...
	if (LsrRegister & XUN_LSR_TX_BUFFER_EMPTY) {
		 ...
		 * are enabled,
		 */
		IirRegister = XIo_In8(InstancePtr->BaseAddress +
							XUN_IIR_OFFSET);
...
unsigned int XUartNs550_ReceiveBuffer(XUartNs550 *InstancePtr)
{
	...
	while (ReceivedCount < InstancePtr->ReceiveBuffer.RemainingBytes) {
		...
		if (LsrRegister & XUN_LSR_BREAK_INT) {
			(void)XIo_In8(InstancePtr->BaseAddress +
							XUN_RBR_OFFSET);
